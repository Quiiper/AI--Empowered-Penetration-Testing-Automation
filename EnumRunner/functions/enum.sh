...
#       create algo to check which version of snmp is runnign or pull it off a banner grab
        ...
        wait
        rm $IP/EnumRunner/loot/raw/snmp_found
}
...
http_enum (){
        mkdir -p $IP/EnumRunner/loot/http
...
smb_enum (){
        ...
        if ! grep -q "smb-vuln-ms08-067:" "EnumRunner/loot/smb/08-067";then rm $loot/smb/08-067;fi
        ...
        find ~ -path '*/$IP/EnumRunner/loot/smb/*' -type f > $loot/smb/files
